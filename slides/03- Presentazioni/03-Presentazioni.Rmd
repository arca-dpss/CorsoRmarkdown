...
- `quarto`: <font size="3">sempre in html, ma mooooolto più carine [05-quarto](https://arca-dpss.github.io/CorsoRmarkdown/slides/05%20-%20quarto/05-quarto.html#/title-slide)</font>

- `xaringan`: <font size="3">sempre in html, ma mooooolto più carine [07-xaringan](https://arca-dpss.github.io/CorsoRmarkdown/slides/07%20-%20xaringan/07-xaringan.html#1)</font>
...
Si può anche evitare di avere un file `css` esterno e scrivere il codice all'interno del documento stesso: 
...
```{r, fig.cap="Come pensavo di preparare il corso", echo =FALSE, eval =TRUE, out.width="70%"}
knitr::include_graphics("img/expectation.jpg")
```


<!-- ```{r echo=FALSE} -->
<!-- knitr::include_graphics("slides/03-Presentazioni/img/expectation.jpg") -->
<!-- ``` -->
...
Usando `{.columns-2}` non si ha molta flessibilità $\rightarrow$ il testo viene automaticamente messo in due colonne
...
- "Troncare" l'output di `R` in modo che rientri comodamente nella slide (lo abbiamo già visto)
- Scrollare il codice
...
Nel chunk di cui si vuole fare scrollare il codice aggiungere l'argomento: `class.output="scroll-100"` ed è fatta!
...
Lo abbiamo già visto per la preparazione dei file `pdf` ma ripassiamo 
